...
\begin{equation*}
	\Pi_\mathcal{X}^\Phi(y) = \argmin_{y\in \mathcal{X}\cap\mathcal{D}} D_\Phi(x, y).
\end{equation*}
...
\begin{align*}
	...
	& \leq \eta L\left\|x_s - x_{s+1}\right\| -\frac{\alpha}{2}\left\|x_s - x_{s+1}\right\|^2 \\
	& \leq \frac{\left(\eta L\right)^2}{2\rho},
\end{align*}
where we used the inequality $az - bz^2 \leq a^2/ab$ for all $z\in \mathbb{R}$ in the last line and $\alpha$-strong convexity in line 4. We compute
\begin{align}
...
\end{equation*}
that we used in the above proof is somewhat crucial in our case because $L$ coincides with the dual or operator norm, i.e. $L = \|g\|_* = \sup\{g^\mathsf{T}x : \|x\|=1\}$. Using the fact that for $1 \leq p, q \leq \infty$ and $\frac{1}{p} + \frac{1}{q} = 1$ the norms $\ell^p$ and $\ell^q$ are dual, we see that we can get much finer control by adjusting the norm we are using to the problem hat hand. Seen from a different point, we can now use any appropriate form of Hölder's inequality and are not confined to using Cauchy-Schwarz.
\end{remark}
