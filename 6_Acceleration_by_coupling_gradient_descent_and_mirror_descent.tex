...
\[
    \Mirr(\xi) := \argmin_{y \in \X} \left\{ D(y, x) + \langle \xi, y-x \rangle \right\}.
\]
...
%TODO mention complexity of mirror descent.
%TODO adapt mirror's descent key lemma (and proof if it wasn't proved in week 5)
...
\[
    \forall u \in \X = \R^n, \quad \eta \langle \nabla f(x_{k+1}), x_{k+1} - u \rangle \leq \eta^2\beta\left( f(y_k) - f(y_{k+1})\right)a + \left( D(u, {z_k}) - D(u, {z_{k+1}})\right).
\]
\end{lemma}
...
\caption{AGM $(f, w, x_0, T)$}
   ...
   \FOR{$ k \gets 0 $ to $T-1$}
       ...
       \STATE{ $z_{k+1} \gets \Mirr_{z_k}\left(\eta_{k+1} \nabla f (x_{k+1})\right)$ \hfill\hfill $\left( := \argmin_{y\in\X} \left\{ D(z, {z_k}) + \langle \eta_{k+1} \nabla f(x_{k+1}), z-z_k \rangle \right\}\right) $}
